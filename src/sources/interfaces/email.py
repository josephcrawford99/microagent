...
The agent sees `email.send(to, subject, body)` and `email.poll()` —
`tools/email.py` derives the send schema from EmailMessage's dataclass fields,
so `subject` appears without a hand-written schema.
"""
